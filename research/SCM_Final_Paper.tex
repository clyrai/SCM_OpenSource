...
\textbf{Direction adopted to reduce single-encoder dependence.} We now treat embeddings as one retrieval surface inside a multi-store memory model: structured profile facts, ephemeral task-context slots, episodic traces, and semantic vectors. Retrieval executes lexical matching, exact slot/entity matching, graph-neighborhood recall, and vector similarity in parallel, then fuses candidates with weighted reciprocal rank fusion plus symbolic boosts for current-version preference, recency, context match, and source confidence. When fused confidence is low, the runtime prefers clarifying questions or bounded uncertainty over forced guesses. These updates are implemented in the product path, but their long-horizon effect on external benchmarks remains to be quantified.
...
In current product integration, this architecture is instantiated as a hybrid retrieval stack rather than a pure vector stack: profile facts and task-context slots are retrieved explicitly, episodic/graph memory is recalled through spreading activation, and vector retrieval provides paraphrase/generalization support as one channel among several. This framing preserves SCM's sleep-consolidation advantages while reducing brittle dependence on any single embedding model.
